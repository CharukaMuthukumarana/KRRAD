...
# Split multiple interfaces by comma
devices = os.environ.get("INTERFACE", "eth0").split(",")
...
try:
    ...
    # Attach the same eBPF program to all specified interfaces
    for device in devices:
        device = device.strip()
        try: b.remove_xdp(device, flags)
        except: pass
        b.attach_xdp(device, fn, flags)
        print(f" Successfully attached KRRAD XDP to {device}")
...
def get_metrics():
    ...
    return jsonify({"packets": total_packets, "bytes": total_bytes, "details": details, "top_source_ip": top_ip_str, "ip_entropy": ip_entropy})
